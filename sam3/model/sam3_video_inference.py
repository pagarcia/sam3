...
class Sam3VideoInference(Sam3VideoBase):
    ...
    def add_mask(
        ...
        raise NotImplementedError(
            "add_mask requires Sam3VideoInferenceWithInstanceInteractivity"
        )
...
class Sam3VideoInferenceWithInstanceInteractivity(Sam3VideoInference):
    ...
    def add_mask(
        ...
        """Add a user seed/refinement mask to Tracker on a single frame."""
        ...
        if obj_rank is None:
            ...
            logger.debug(
                f"[rank={self.rank}] Adding new object with id {obj_id} from mask at frame {frame_idx}."
            )
            self.add_action_history(
            ...
            logger.debug(
                f"[rank={self.rank}] Refining existing object with id {obj_id} from mask at frame {frame_idx}."
            )
            self.add_action_history(
        ...
        if self.rank == obj_rank:
            frame_idx, obj_ids, low_res_masks, video_res_masks = self.tracker.add_new_mask(
            ...
            # IMPORTANT:
            # Do NOT call clear_detector_added_cond_frame_in_tracker() here.
            # This mask is itself the user conditioning frame.

        if self.rank == obj_rank and video_res_masks is not None and len(obj_ids) > 0:
