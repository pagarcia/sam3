...
class Sam3VideoPredictor:
    ...
    def handle_stream_request(self, request):
        ...
        if request_type == "propagate_in_video":
            return_direction = request.get("propagation_direction", "both")
            yield from self.propagate_in_video(
                session_id=request["session_id"],
                propagation_direction=return_direction,
    ...
    def start_session(self, resource_path, session_id=None):
        """
        Start a new inference session on an image or a video.
        """
    ...
    def add_mask(
        ...
        frame_idx, outputs = self.model.add_mask(
            inference_state=inference_state,
            frame_idx=frame_idx,
            mask=mask,
            obj_id=obj_id,
